...
export class SqlitePlaylistRepository implements PlaylistRepository {
  ...
  async save(playlist: Playlist): Promise<void> {
    ...
    await this.db.runAsync(
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [p.id, p.name, p.artworkUri, p.isUserCreated, p.trackCount, p.createdAt, p.updatedAt]
    );
  }
  ...
  async findAll(): Promise<Playlist[]> {
    const rows = await this.db.getAllAsync<any>("SELECT * FROM playlists ORDER BY createdAt DESC");
    return rows.map(PlaylistMapper.toDomain);
  }

  async findById(id: string): Promise<Playlist | null> {
    const row = await this.db.getFirstAsync<any>("SELECT * FROM playlists WHERE id = ?", [id]);
